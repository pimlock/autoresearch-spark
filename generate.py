...
def generate(model, tokenizer, prompt, max_new_tokens=200, temperature=1.0, top_k=50, device="cuda"):
    model.eval()
...
def main():
    ...
    print(f"Loaded ({config.n_layer}L {config.n_embd}d, val_bpb={ckpt.get('val_bpb', '?'):.4f})")

    tokenizer = Tokenizer.from_directory()

    if args.prompt is not None:
        output = generate(model, tokenizer, args.prompt, args.max_new_tokens, args.temperature, args.top_k, device)
        print(f"\n{args.prompt}{output}")
    else:
