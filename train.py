...
# Model architecture
ASPECT_RATIO = 96       # model_dim = depth * ASPECT_RATIO (slightly wider)
HEAD_DIM = 64           # target head dimension for attention
WINDOW_PATTERN = "L"    # all full-context attention (no sliding window)

# Optimization
TOTAL_BATCH_SIZE = 2**14 # ~16K tokens per optimizer step (32x more steps than baseline)
EMBEDDING_LR = 0.3      # learning rate for token embeddings (Adam) - reduced
UNEMBEDDING_LR = 0.004  # learning rate for lm_head (Adam)
MATRIX_LR = 0.06        # learning rate for matrix parameters (Muon) - increased
SCALAR_LR = 0.5         # learning rate for per-layer scalars (Adam)
WEIGHT_DECAY = 0.0      # no weight decay (try simpler)
...
def get_muon_momentum(step):
    frac = min(step / 5000, 1)  # very slow warmup - never fully reaches 0.95
